...
def _ensure_cpuinfo_module(applied: list[str]) -> None:
    try:
        importlib.import_module("cpuinfo")
        return
    except ImportError:
        cpuinfo_module = _ensure_module("cpuinfo")
    ...
    if _ensure_attr(cpuinfo_module, "get_cpu_info", get_cpu_info):
        applied.append("cpuinfo.get_cpu_info")
...
def patch_for_vllm_import() -> tuple[str, ...]:
    ...
    _ensure_cpuinfo_module(applied)
